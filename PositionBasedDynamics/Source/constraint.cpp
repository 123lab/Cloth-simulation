...
{// TODO: implement the project function for FixedPointConstraint.
    ...
    //float value = 0.0f;
	glm::vec3 p0;
	p0 = m_vertices->predicted_pos(m_p0);

	float value = glm::length(m_fixd_point - p0);
    if(value < EPSILON)
        return true;
...
{// TODO: implement the project function for StretchConstraint.
	...
	float diff = length - m_rest_length;
	...
	//float a = m_vertices->inv_mass(m_p1)/(m_vertices->inv_mass(m_p1) + m_vertices->inv_mass(m_p2));
	//float b = m_vertices->inv_mass(m_p2)/(m_vertices->inv_mass(m_p1) + m_vertices->inv_mass(m_p2));

	float sum = (w1+w2);
	glm::vec3 temp = (diff) * (p1-p2) / length;
	dp1 = - w1/sum * temp;
	dp2 = w2/sum * temp;
