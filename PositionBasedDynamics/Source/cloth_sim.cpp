...
{// TODO: generate all internal constraints in this function.
    ...
    for(i = 0; i < m_dimx; ++i)
    {
        for(k = 0; k < m_dimz; ++k)
        {
			...
			//if(index == (m_dimz-1) * (m_dimz)  || index == 0){
			if(k==0){
				FixedPointConstraint* fixConstraint = new FixedPointConstraint(&m_vertices, index, m_vertices.pos(index)); //20140228
				m_constraints_int.push_back(fixConstraint);
			}
    ...
    {// TODO: add stretch constraint here.
		...
		StretchConstraint* stretchConstraint = new StretchConstraint(&m_vertices, s_stiff, p1_index, p2_index, glm::length(m_vertices.pos(p1_index)-m_vertices.pos(p2_index)) );  //20140307
...
void ClothSim::damp_velocity(float k_damp)
{
	...
	int index;
    glm::vec3 xcm;
	glm::vec3 num_x = glm::vec3(0.0f,0.0f,0.0f);
	float den_x = 0.0f;
	...
	for(int i = 0; i < m_dimx; ++i)
    {
        ...
        {
            index = m_dimz * i + k;
            num_x = num_x + m_vertices.pos(index) / m_vertices.inv_mass(index);
			den_x = den_x + 1.0/m_vertices.inv_mass(index);
        }
    }
	xcm = num_x/den_x;
	//(2)
	glm::vec3 vcm;
	glm::vec3 num_v = glm::vec3(0.0f,0.0f,0.0f);
	float den_v = 0.0f;
	...
	for(int i = 0; i < m_dimx; ++i)
    {
        ...
        {
            index = m_dimz * i + k;
            num_v = num_v + m_vertices.vel(index) / m_vertices.inv_mass(index);
			den_v = den_v + 1.0/m_vertices.inv_mass(index);	
        }
    }
	vcm = num_v/den_v;
	//(3)(4)
	...
	for(int i = 0; i < m_dimx; ++i)
    {
        for(int k = 0; k < m_dimz; ++k)
        {
			...
			glm::mat3 r_skew = glm::mat3(v1,v2,v3);
			glm::mat3 r_skew_t = glm::transpose(r_skew);
			I = I + r_skew * r_skew_t / m_vertices.inv_mass(index);           
        }
    }
	//(5)
	...
	for(int i = 0; i < m_dimx; ++i)
    {
        for(int k = 0; k < m_dimz; ++k)
        {
			...
			m_vertices.vel(index) = m_vertices.vel(index) + k_damp * dv;
		}
    }
